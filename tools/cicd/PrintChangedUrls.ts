...
function removeIndexTsx(input: string): string {
  ...
  if (input.endsWith('.module.less')) return input.substr(0, input.length - '.module.less'.length)
  if (input.endsWith('.less')) throw new Error(`You should use LESS Modules instead of ${input}`)
  return input
}
