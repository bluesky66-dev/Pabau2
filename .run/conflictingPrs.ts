...
(async () => {
  ...
  for (const e of bbList) {
    ...
    const checkout = child_process.spawnSync(`git`, [`checkout`, `${e.source.commit.hash}`]);
    ...
    const merge = child_process.spawnSync(`git`, [`merge`, `origin/master`, `--no-commit`, '--no-ff']);
    if (merge.status > 0) {
      //console.log(`${e.author.display_name}: ${e.source.branch.name}`);
      merge.stdout.toString().split('\n').forEach(e => {
      ...
      const approvals = e.participants?.reduce((a,c)=>a+(c.approved ? 1 : 0), 0)
      console.log(`${e.author.display_name}: ${e.source.branch.name}: ${approvals}: ${e.links.html.href}`);
    }
