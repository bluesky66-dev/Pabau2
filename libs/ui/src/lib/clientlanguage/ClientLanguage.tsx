...
const LanguagePop: FC<P> = ({ childHook: [preferredLang, setPreferredLang] }) => {
	const onClickLang = (index) => {
		const preferredLangTemp = [...preferredLang]
		if (!preferredLangTemp.find((item) => item.shortLabel === languageMenu[index].shortLabel)) {
			...
			{languageMenu.map((item, index) => (
				...
				>
					<div className={styles.languageLeft}>
						<Image src={item.logo} width={16} alt={item.label} preview={false} />
						<span className={item.selected ? styles.languageNameSelected : styles.languageName}>
							{item.label}
						</span>
					</div>
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.popover}>
				<Popover
					content={<LanguagePop childHook={[preferredLang, setPreferredLang]} />}
					placement="topLeft"
					trigger="click"
				>
